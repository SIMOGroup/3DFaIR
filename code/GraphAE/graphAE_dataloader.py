...
##return batch*p_num*3 torch   batch*p_num*3 torch
def get_random_pc_batch_from_pc_lst_torch_X_yhat(pc_lst_X, pc_lst_Y_hat , neighbor_lst, neighbor_num_lst, batch, augmented=False):
    
    pcs_index_lst = np.random.randint(0, len(pc_lst_X), batch)
    pcs_X = pc_lst_X[pcs_index_lst]
    pcs_Y_hat = pc_lst_Y_hat[pcs_index_lst]
    pc_batch_X = torch.FloatTensor(pcs_X).cuda()
    pc_batch_Y_hat = torch.FloatTensor(pcs_Y_hat).cuda()

    if(augmented==True):
        pc_batch_X=get_augmented_pcs(pc_batch_X)
        pc_batch_Y_hat=get_augmented_pcs(pc_batch_Y_hat)
    
    return pc_batch_X, pc_batch_Y_hat
...
#pc p_num np*3
#template_ply Plydata
def save_pc_into_ply_with_faces(faces, pc, fn):
    data=trimesh.Trimesh(pc, faces)
    #pc = pc.copy()*pc_std + pc_mean
    # plydata['vertex']['x']=pc[:,0]
    # plydata['vertex']['y']=pc[:,1]
    # plydata['vertex']['z']=pc[:,2]
