...
def test(param,test_npy_fn, gt_npy_fn, out_ply_folder, skip_frames =0):
    ...
    while (n<(pc_num-1)):
        ...
        pcs_gt_list = pc_gt_list[n:n+batch]
        height = pcs[:,:,1].mean(1)
        pcs[:,:,0:3] -= pcs[:,:,0:3].mean(1).reshape((-1,1,3)).repeat(param.point_num, 1) ##centralize each instance
        pcs_gt_list[:,:,0:3] -= pcs_gt_list[:,:,0:3].mean(1).reshape((-1,1,3)).repeat(param.point_num, 1)
        ...
        if(n % 1 ==0):
            ...
            pc_out[:,1] +=height[0]
            pc_y = np.array(pcs_gt_list[0].data.tolist())
            pc_y[:,1] +=height[0]
            ...
            Dataloader.save_pc_into_ply(template_plydata, pc_gt, out_ply_folder+"%08d"%(n)+"_in.ply")
            Dataloader.save_pc_into_ply(template_plydata, pc_y, out_ply_folder+"%08d"%(n)+"_gt.ply")
...
with torch.no_grad():
    ...
    test(param, test_npy_fn, gt_npy_fn, out_ply_folder,skip_frames=0)
